        ...
        for (var i = 0; i < count; i++)
         {
            ...
            $('#salesmaninfodata').append('<tr><td class="text-center" >' + uploadimg+ '</td><td class="text-center"><button type="button" class="btn btn-link" data-toggle="collapse" title="Update Salesman Information" data-target="#demo" onclick="updatesalesinfo(' + sales_id + ');">' + data[i]['fullname'] + '</button></td><td class="text-center">' + data[i]['email'] + '</td><td class="text-center">'+ data[i]['mobile'] +'</td><td class=" text-center">' + data[i]['status'] +
						'</td><td class=" text-center">' + data[i]['address'] + '</td><td class=" text-center"><div class="btn-group"><button type="button" class=" btn btn-info " data-toggle="collapse" title="Update Salesman Information"  name="submit"  onclick="updatesalesinfo(' + sales_id + ');"><i class="fa fa-pencil"></i></button><button type="button" class=" btn btn-danger" title="Remove Salesman" name="submit"  id="' + sales_id + '" onClick="removesalesman(' + sales_id + ');" ><i class="fa fa-trash-o"></i></button><button type="button" class=" btn btn-primary " title="" name="submit"  id="' + sales_id + '" onClick="Assignwork(' + sales_id + ');" ><i class="fa fa-thumbs-up"></i></button></div></td></tr>');
        }
       }
...
function removesalesman(param)
{
  ...
  if (t == true) {
            $.ajax({
            url:"../src/deletesalesman.php",
            async: false,
            cache: false,
            method:"POST",
            data:({removesalesman:param,tblName:'SalesManMaster',colName:'salesManId'}),
            success:function(data)
            {
...
function removeshopsman(param)
{
  ...
  if (t == true) {
            ...
            cache: false,
            method:"POST",
            data:({removesalesman:param,tblName:'shopKeeperMaster',colName:'shopKeeperId'}),
            success:function(data)
            {
              $("#"+param).closest('tr').remove();
                window.location.reload();
...
function Assignwork()
{
	window.location.href="assign-work.php";
}
function display_shopkeeper()
{
	...
	'</td><td class=" text-center">' + data[i]['address'] + '</td><td class=" text-center"><div class="btn-group"><button type="button" class="btn btn-info " data-toggle="collapse" title="Update Shopman Information" data-target="#hidecustomerfield" name="submit"  onclick="updateshopsinfo(' + shops_id + ');"><i class="fa fa-pencil"></i></button><button type="button" class="btn btn-danger" title="Remove Salesman" name="submit"  id="' + shops_id + '" onClick="removesalesman(' + shops_id + ');" ><i class="fa fa-trash-o"></i></button><button type="button" class="btn btn-primary" title="" name="submit"  id="' + shops_id + '" onClick="Assignwork(' + shops_id + ');" ><i class="fa fa-thumbs-up"></i></button></div></td></tr>');
			}
		 }
		$("#datatable-default").DataTable({
		 });
